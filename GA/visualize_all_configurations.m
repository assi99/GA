function visualize_all_configurations(stl_file, prop_positions, idx_log, cog_xy)
    ...
    % Ploting all the evaluated propeller positions
    for k = 1:length(idx_log)
        ...
        scatter(p1(1), p1(2), 20, color_p1, 'filled');  % red
        scatter(p2(1), p2(2), 20, color_p2, 'filled');  % green
        scatter(p3(1), p3(2), 20, color_p3, 'filled');  %  blue
    end

    % Ploting the COG
    scatter(cog_xy(1), cog_xy(2), 50, 'k', 'x', 'LineWidth', 2);  % A black X
